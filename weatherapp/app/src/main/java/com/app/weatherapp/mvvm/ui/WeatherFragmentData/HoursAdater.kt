...
class HoursAdapter(val context:Context): RecyclerView.Adapter<HoursAdapter.WeatherViewHolder>() {
    var weatherList: List<Hourly> =ArrayList()
    ...
    override fun onBindViewHolder(holder: WeatherViewHolder, position: Int) {
        ...
        holder.tvDate.text = SimpleDateFormat("hh:mm a", Locale.ENGLISH).format(Date(((weatherList[position].dt).toLong()*1000)))
        var sp= PreferenceManager.getDefaultSharedPreferences(context)
        if(sp.getString("UNIT_SYSTEM", "")=="STANDARD") {
            holder.weatherTemp.text = weatherList[position].temp.toString()+"°K"
        }
        else if(sp.getString("UNIT_SYSTEM", "")=="METRIC")
        {
            holder.weatherTemp.text = weatherList[position].temp.toString()+"°C"

        }
        else
        {
            holder.weatherTemp.text = weatherList[position].temp.toString()+"°F"
    ...
    fun setData(weatherList: List<Hourly>) {
